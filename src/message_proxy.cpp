...
namespace parser {
    ...
    MessageProxy::MessageProxy(const char* p, std::size_t offset, Topic topic,
        Action action, bool is_ack)
        : MessageProxy(p, offset, Header(topic, action, is_ack))
    {
    }

    MessageProxy::MessageProxy(const char* p, std::size_t offset,
    ...
    std::size_t MessageProxy::size_impl_() const { return size_; }

    const Message::Header& MessageProxy::header_impl_() const { return header_; }
