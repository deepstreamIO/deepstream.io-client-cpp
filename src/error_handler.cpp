...
void ErrorHandler::authentication_error(const Message& message)
{
    ...
    assert(message.action() == Action::ERROR_INVALID_AUTH_DATA || message.action() == Action::ERROR_INVALID_AUTH_MSG || message.action() == Action::ERROR_TOO_MANY_AUTH_ATTEMPTS);
...
void ErrorHandler::invalid_state_transition_impl(client::State s,
    const Message& msg)
{
...
{
    std::fprintf(stderr, "Too many redirections (%u redirections)\n",
        max_num_redirections);
}

void ErrorHandler::system_error_impl(const std::system_error& e)
{
    std::fprintf(stderr, "System error: %s [code=%d]\n", e.what(),
        e.code().value());
}

void ErrorHandler::invalid_close_frame_size_impl(
    const websockets::Frame& frame)
{
    std::fprintf(stderr,
...
{
    std::fprintf(stderr, "WebSocket exception [msg=%s]\n", e.what());
}

void ErrorHandler::unexpected_websocket_frame_flags_impl(int flags)
{
    std::fprintf(stderr, "Unexpected WebSocket frame flags [flags=%d]\n", flags);
}

void ErrorHandler::sudden_disconnect_impl(const std::string& uri)
{
    std::fprintf(stderr, "Sudden disconnect [uri=%s]\n", uri.c_str());
}
...
void ErrorHandler::authentication_error_impl(const Message& msg)
{
    ...
    std::fprintf(stderr, "Authentication error: '%s'\n", str.c_str());
}
}
