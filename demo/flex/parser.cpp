...
int deepstream_parser_handle(deepstream::parser::State* p_state,
    deepstream_token token, const char* match,
    std::size_t matchlen)
{
    assert(p_state);
    assert(match);
    ...
    if (token == TOKEN_UNKNOWN) {
        fprintf(stderr, "Unknown token %zu bytes into the message\n", offset);
    ...
    if (p_state->tokenizing_header && !is_header_token(token)) {
        fprintf(stderr, "Expected header %zu bytes into the message\n", offset);
    ...
    if (!p_state->tokenizing_header && token == TOKEN_EOF) {
        fprintf(stderr, "Unexpected EOF %zu bytes into the message\n", offset);
